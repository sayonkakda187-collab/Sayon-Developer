...
export function FacebookPagesManager({
  ...
  // Totals for the stat gauges (mirrors the Dashboard tab's summary row).
  ...
  return (
    <div>
      ...
      {/* Totals summary — same animated gauges as the Dashboard tab. */}
      {pages.length > 0 && (
        <div className="adm-stats" style={{ marginTop: 14 }}>
          <StatGauge
            value={totals.pages}
            label="Total Pages"
            sub={`${grouped.length} group${grouped.length === 1 ? "" : "s"}`}
