...
// Publishes every article whose scheduled time has arrived (+ fires the Facebook
// auto-share THEN). Idempotent — safe to call as often as the external pinger
// likes; it never double-publishes or double-shares.
...
export async function GET(req: Request) {
  ...
  try {
    const summary = await publishDue();
