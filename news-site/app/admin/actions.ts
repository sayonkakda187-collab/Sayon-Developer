...
export async function saveArticle(formData: FormData) {
  ...
  // Auto-generate the "Key Points" box on the FIRST publish, only when the editor
  // left it empty and AI is configured. Best-effort + time-boxed: a failure (or
  // missing key) never blocks publishing — the admin can click "Generate key
  // points" later. Skipped when the admin already wrote/generated points.
  if (freshlyPublished && !keyPoints && isAiConfigured()) {
    try {
      const model = await getDefaultAiModel();
      const points = await generateKeyPoints({ title, body: content, model });
      if (points.length > 0) {
        await prisma.article.update({
          where: { id: savedId },
          data: { keyPoints: points.join("\n") },
        });
      }
    } catch {
