...
// Admin-only: visitor-country + device stats for the Audience dashboard. Returns
// mapped counts only (no IP/PII is ever stored or returned).
export async function getAudienceStats(input: {
  ...
  stats: CountryStat[];
  total: number;
  devices: { stats: DeviceStat[]; total: number };
}> {
  await requireAdmin();
  ...
  const [countries, devices] = await Promise.all([
    getCountryStats({ articleId, days }),
    getDeviceStats({ articleId, days }),
  ]);
