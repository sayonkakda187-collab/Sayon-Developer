...
- **The unit:** Adsterra **Social Bar** — a self-displaying format. One loader
  ...
  there is **no container element to place**; mounting it once is the whole
  integration.
- **Where:** `components/AdsterraSocialBar.tsx`, mounted as the **last child** of
  `app/(public)/layout.tsx` — the final thing in `<body>`, covering all eight
  public routes (home · article · category · search · about · contact · privacy ·
  terms). **Never on `/admin`, `/portal/[token]` or `/g/[token]`:** an ad bar over
  the editor is in the way, and impressions generated by the site's own operator
  are exactly what ad networks classify as invalid traffic.
- **Non-blocking:** `next/script` with **`strategy="afterInteractive"`** + `async`
  — the App Router equivalent of "put it right before `</body>`". Next emits only
  a `<link rel="preload" as="script">` into `<head>` (a fetch *hint*, never
  awaited) and injects the real `<script>` into `<body>` after hydration.
  Verified in Chromium: with the ad host **stalled 6s**, the homepage still hit
  DOMContentLoaded in **81ms** and painted its headline in **216ms**.
