...
## Ads (AdsKeeper only — every Adsterra unit removed)

**AdsKeeper is the only network on the site.** Every Adsterra unit was removed at
the owner's request: the Social Bar, Popunder, Native Banner, in-article 300x250
and the sticky bottom banner, plus the older `AdsterraScripts` / `AdsterraBanner`
/ `AdsterraNative` integration and `lib/adsterra.ts`. `lib/articleSplit.ts` and
`npm run check:split` went with them — the splitter existed ONLY to position the
300x250, and the AdsKeeper layout does its own placement (`buildArticleParts`).
**Don't reintroduce any of it without asking.**

- **Everything you edit lives in `lib/ads.ts`** — site ids, one widget id per
  placement, and `ADS_ENABLED`. It is heavily commented; read it before changing
  a placement. The ids are public by design (they ship in the HTML), so they are
  committed rather than hidden in env vars.
- **Two registered sites, chosen per REQUEST HOST.** `adsForHost(host)` returns
  the site id + widget set: **ledgerdailynews.com → site `1108814`** (the current
  primary) and **dailyledger.today → site `1097964`** (the previous domain, still
...
### The placements (ledgerdailynews.com, site 1108814)
...
- **`SIDEBAR_1` is desktop-only by construction**: `AdRail` never mounts below
  1024px, so a phone never receives the container and an impression is never
  served to a reader who cannot see it.
- **`NOTIFICATION` is deliberately EMPTY, and must stay that way** unless a
  widget genuinely positions itself. `AdOverlay` renders a bare container at the
  very end of the layout, after the footer — correct only for a format that then
  repositions itself. Widget `2071266` was typed "In-site notification" in the
  dashboard but rendered INLINE, which put a "Promoted content" block under the
  copyright line. **The dashboard type is not sufficient evidence — confirm on
  the live site that a widget floats before putting its id here.**
- `SECTION_ADS_PRIMARY` (an ad under each `## ` section) is empty: it needs one
...
absent on a 390px phone, and no horizontal overflow at 9 widths across 4 page
types.


### Google AdSense (verification signals only — no ad units)

No AdSense ad renders anywhere; only the ownership signals remain, so the pending
application isn't invalidated.

- **Verification:** a **server-rendered `<meta name="google-adsense-account">`**
  in `<head>`, emitted via **`metadata.other`** in the root layout
  (`app/layout.tsx`). This is Google's recommended signal and is guaranteed to be
  in the **RAW server HTML** the crawler reads without executing JS. (An earlier
  `next/script` tag alone did **not** pass verification.) Inherited by every route.
- **Publisher id:** `ADSENSE_PUBLISHER_ID` in `lib/ads.ts`
  (`ca-pub-5470257305108580`) — public by design; it ships in the HTML and must
  match the `google.com, pub-…, DIRECT` line in `public/ads.txt`.
- **ads.txt:** `public/ads.txt` (served at `/ads.txt`) declares authorized
  sellers. Harmless; never overwrite it.
- The **`adsbygoogle.js` library script was removed** with the slots. Add it back
  only when real `<ins class="adsbygoogle">` units ship after approval.
- ⚠️ Google verifies on the **production domain** only — changes must be merged
  and deployed, not just sitting on a preview URL.

### AdsKeeper earnings dashboard (reporting only — not an ad)

`lib/adskeeper/` plus the admin panel, settings and balance pill are **still
present**. They read revenue figures from the AdsKeeper publisher API and
...
Verified 320/344/345/360/375/393/430/768/1280 on home, article, category and
search by `verify-adskeeper.mjs` §8: `scrollWidth` never exceeds the viewport and
Search is on screen at every width. That coverage used to live in a suite built
around the Adsterra native banner; when those units were removed it moved here,
widened from one page to four, so the protection did not lapse with them.
**Anything that makes the page wider than the viewport also breaks the AdsKeeper
sticky footer**, which is `position: fixed; width: 100%` and therefore spans the
*layout* viewport — when the page overflows, its ✕ ends up off-screen.

## Site extras (Key Points · Breaking banner · Markets ticker)

Three reader/ops features added together; each integrates with the existing
article model, AppSetting store, AI pipeline, and public layout. (A fourth —
reserved AdSense slots — was removed with the rest of the ad code; see **Ads**.)
...
- **Landing** (`PageControlList`): shows **ONLY** `MonitoredPage`s (searchable, avatar'd,
  ...
  **header** bar: on the `/admin/page-control` list route ONLY, `AdminShell` swaps the global
  ...
  `PageControlHeaderControls`: a **"Search by manager" AUTOCOMPLETE** + the **"Connect Page"**
  button (bumps `pageControlConnectStore`; the list opens its existing connect modal on the
  signal). The autocomplete opens on focus (all managers — avatar + name + page count, from
  ...
  date-range chips (no in-box Connect/manager-search bars).
